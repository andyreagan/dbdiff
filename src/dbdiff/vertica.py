...
def get_column_info(cur: Cursor, schema_name: str, table_name: str) -> pd.DataFrame:
    column_template = JINJA_ENV.get_template("table_columns.sql")
    cur.execute(column_template.render(schema_name=schema_name, table_name=table_name))
...
def get_column_info_lookup(cur: Cursor, schema_name: str, table_name: str) -> dict:
    x_table_info = get_column_info(cur, schema_name, table_name)
    LOGGER.info(
        "Column info Vertica for "
        + schema_name
        + "."
        + table_name
        + ":\n"
...
def get_table_exists(cur: Cursor, schema_name: str, table_name: str) -> bool:
    table_exists_template = JINJA_ENV.get_template("table_exists.sql")
    cur.execute(table_exists_template.render(schema_name=schema_name, table_name=table_name))
...
def implicit_dtype_comparison(x_dtype: str, y_dtype: str) -> bool:
    ...
    if "INT" in source:  # {'INT', 'INTEGER'} since numeric can look like NUMERIC(18,2), etc
        return "BOOL" in target or "NUMERIC" in target or "FLOAT" in target or "INT" in target
    elif "NUMERIC" in source:
        return "FLOAT" in target or "NUMERIC" in target
    # don't worry about LONG VARCHAR not converting to VARCHAR right now
    elif "CHAR" in source:
        return "CHAR" in target or "FLOAT" in target
...
def get_cur() -> Cursor:
    ...
    if os.environ.get("CERT_LINK") is not None:
        ...
        if (use_ssl_env is not None) and (
            (use_ssl_env == "1") or (use_ssl_env.lower() in {"true", "yes", "please"})
        ):
