...
def create_x_table(cur):
    cur.execute(
        "CREATE TABLE dbdiff.x_table ( join1 varchar(10), join2 varchar(10), missingx int, missingx2 int, dtypemiss int, data1 int, data2 int, data3 date, data4 varchar(10));"
    )
    cur.execute(
        "INSERT INTO dbdiff.x_table ( join1, join2, missingx, missingx2, dtypemiss, data1, data2, data3, data4) (select 'match1', 'matchdup21', 0, 0, 0, 0, 0, '2017-10-11', '');"
    )
    cur.execute(
        "INSERT INTO dbdiff.x_table ( join1, join2, missingx, missingx2, dtypemiss, data1, data2, data3, data4) (select 'match1', 'match22', 0, 0, 0, 0, 0, '2017-10-11', 'a');"
    )
    cur.execute(
        "INSERT INTO dbdiff.x_table ( join1, join2, missingx, missingx2, dtypemiss, data1, data2, data3, data4) (select 'match1', 'match23', 0, 0, 0, 1, 1, '2017-10-11', '');"
    )
    cur.execute(
        "INSERT INTO dbdiff.x_table ( join1, join2, missingx, missingx2, dtypemiss, data1, data2, data3, data4) (select 'match1', 'missx21', null, null, null, null, null, null, '');"
    )
    cur.execute(
        "INSERT INTO dbdiff.x_table ( join1, join2, missingx, missingx2, dtypemiss, data1, data2, data3, data4) (select 'match1', 'missx22', null, null, null, null, null, null, '');"
    )
    cur.execute(
        "INSERT INTO dbdiff.x_table ( join1, join2, missingx, missingx2, dtypemiss, data1, data2, data3, data4) (select 'missx11', null, null, null, null, null, null, null, '');"
    )
    cur.execute(
        "INSERT INTO dbdiff.x_table ( join1, join2, missingx, missingx2, dtypemiss, data1, data2, data3, data4) (select 'missx12', null, null, null, null, null, null, null, '');"
    )
    cur.execute(
        "INSERT INTO dbdiff.x_table ( join1, join2, missingx, missingx2, dtypemiss, data1, data2, data3, data4) (select null, null, null, null, null, null, null, null, '');"
    )
    cur.execute("COMMIT;")
...
def create_y_table(cur, case_off: bool = False):
    cur.execute(
        "CREATE TABLE dbdiff.y_table ( join1 varchar(10), join2 varchar(10), missingy int, dtypemiss date, data1 int, data2 int, data3 date, data4 varchar(10));"
    )
    cur.execute(
        "INSERT INTO dbdiff.y_table ( join1, join2, missingy, dtypemiss, data1, data2, data3, data4) (select 'match1', 'matchdup21', 0, '2019-04-22', 0, 0, '2017-10-11', '');"
    )
    cur.execute(
        "INSERT INTO dbdiff.y_table ( join1, join2, missingy, dtypemiss, data1, data2, data3, data4) (select 'match1', 'matchdup21', 0, '2019-04-22', 0, 0, '2017-10-11', '');"
    )
    if case_off:
        ...
        cur.execute(
            "INSERT INTO dbdiff.y_table ( join1, join2, missingy, dtypemiss, data1, data2, data3, data4) (select 'match1', 'match22', 0, '2019-04-22', 0, 1, '2017-10-12', 'A');"
        )
    else:
        cur.execute(
            "INSERT INTO dbdiff.y_table ( join1, join2, missingy, dtypemiss, data1, data2, data3, data4) (select 'match1', 'match22', 0, '2019-04-22', 0, 1, '2017-10-12', 'a');"
    ...
    cur.execute(
        "INSERT INTO dbdiff.y_table ( join1, join2, missingy, dtypemiss, data1, data2, data3, data4) (select 'match1', 'match23', 0, '2019-04-22', 0, 0, '2017-10-13', '');"
    )
    cur.execute(
        "INSERT INTO dbdiff.y_table ( join1, join2, missingy, dtypemiss, data1, data2, data3, data4) (select 'match1', 'missy21', 0, '2019-04-22', 0, 0, null, '');"
    )
    cur.execute(
        "INSERT INTO dbdiff.y_table ( join1, join2, missingy, dtypemiss, data1, data2, data3, data4) (select 'missy11', null, 0, '2019-04-22', 0, 0, null, '');"
    )
    cur.execute("COMMIT;")
...
def test_select_distinct_rows(cur):
    ...
    for use_temp_tables in {True, False}:
        ...
        cur.execute(
            "select * from {schema}.{table}".format(schema=new_table_schema, table="x_table_dup")
        )
...
def test_create_diff_table(cur):
    create_diff_table(
        cur, "dbdiff", "x_table_DIFF", ["join1", "join2"], pd.concat([COMPARE_COLS, JOIN_COLS])
    )
...
def test_get_diff_rows(cur):
    diff_summary = get_diff_rows(cur, "dbdiff", "x_table", ["join1", "join2"], 100)
...
def test_get_column_diffs(cur):
    ...
    for column_name in expected.keys():
        ...
        for i in {0, 1}:
            ...
            assert (
                expected[column_name]["df_raw_shape"][i]
                == grouped_column_diffs[column_name]["df_raw"].shape[i]
            )
            assert (
                expected[column_name]["df_h_x_shape"][i]
                == grouped_column_diffs[column_name]["df_h_x"].shape[i]
            )
            assert (
                expected[column_name]["df_h_y_shape"][i]
                == grouped_column_diffs[column_name]["df_h_y"].shape[i]
            )
...
def test_main(cur):
    ...
    runner_wrapper(
        runner, both_table_temp_options, ["--drop-output-tables", "--output-format=XLSX"]
    )
    ...
    runner_wrapper(
        runner,
        both_table_temp_options,
        ["--save-column-summary", "--save-column-summary-format=pickle"],
    )
